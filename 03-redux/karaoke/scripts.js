...
const renderLyrics = () => {
  ...
  const lyricNumberDisplay = document.getElementById('lyric-number');
  while (lyricsDisplay.firstChild) {
    lyricsDisplay.removeChild(lyricsDisplay.firstChild);
  }
  while (lyricNumberDisplay.firstChild) {
    lyricNumberDisplay.removeChild(lyricNumberDisplay.firstChild);
  }
  ...
  if (store.getState().currentSongId) {
    ...
    const currentLineNumber = document.createTextNode((store.getState().songsById[store.getState().currentSongId].arrayPosition + 1) + ' / ' + store.getState().songsById[store.getState().currentSongId].songArray.length);
    document.getElementById('lyric-number').appendChild(currentLineNumber);
  }
};
